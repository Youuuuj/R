...
# 고급 시각화
# lattice 패키지: 서로 상관있는 확률적 반응변수의 시각화에 사용. 
# 특정 변수가 갖는범주(domain)별로 독립된 패널을 격자(lattice)처럼 배치하여 
# 여러 개의 변수에 대한 범주를 세부적으로 시각화해주는 도구
# ggplot2 패키지: 기하학적 객체들(점, 선, 막대 등)에 미적 특성(색상, 모양, 크기)을 적용하여시각화는 방법을 제공. 
# 그래프와 사용자 간의 상호작용(interaction)기능을 제공하기 때문에 시각화 과정에서 코드의 재사용성이 뛰어나고, 데이터 분석을 위한 시각화에 적합
...
# plot.Points = T : 밀도 점 표시 여부(표시 x -> F)
...
# 3단계 : 데이터 형식 변경(matrix을 table으로 변경)
dft <- as.data.frame.table(VADeaths)
str(dft)
dft
# 통계처리를 위해 table자료구조로 변환
# table구조로 변경하면 가장 왼족의 첫번째 컬럼을 기준으로 '넓은 형식'의 자료가
# '긴 형식'의 자료로 구조가 변경된다.
...
# airquality셋으로 산점도 그래프 그리기
# 1단계 : 데이터셋 가져오기
library(datasets)
str(airquality)

# 2단계 : xyplot()함수를 이용하요 산점도 그리기
xyplot(Ozone ~ Wind, data = airquality)

# 3단계 : 조건변수를 사용하는 xyplot()함수로 산점도 그리기
xyplot(Ozone ~ Wind | Month, data = airquality)

# 4단계 : 조건변수와  layout속성을 사용하느 xyplot() 함수로 산점도 그리기
xyplot(Ozone ~ Wind | Month, data = airquality, layout = c(5,1))

# 5단계 : Month변수를 Factor타입으로 변환하여산점도 그리기
...
tplot <- update(tplot, main = '1964년 이후 태평야엥서 발생한 지진 위치')
print(tplot)


# 이산형 변수를 조건으로 지정하여 산점도 그리기
# 수심별로 진앙지를 파악하기 위해서 depth변수(이산형변수)를 조건으로 지정

# 1단계 : depth 변수의 범위 확인
range(quakes$depth)

# 2단계 : depth 변수 리코딩 - 6개의 범주(100단위)로 코딩변경
...
# 3단계 : 리코딩 된 변수(depth2)를 조건으로 산점도 그리기
convert <- transform(quakes, depth2 = factor(depth2))
xyplot(lat ~ long | depth2, data = convert)


# 동일한 패널에 두 개이 변수값 표현하기
xyplot(Ozone + Solar.R ~ Wind | factor(Month),
       data = airquality,
       col = c('blue', 'red'),
       layout = c(5,1))
# 두개의 변수값을 표현하기 위한 xyplot함수
# 형식 : xyplot(y1축 + y2축 ~ x축 | 조건변수, data, type, layout)
...
# equal.count함수를 사용하여 이산형 변수 범주화하기
# 1단계 : 1~150을 대상으로 겹치지 않게 4개 영역으로 범주화
...
# 이산형 변수를 리코딩한 뒤애 factor형으로 변환하여 산점도 그리기
# 1단계 : depth변수 리코딩
...
# 1단계 : 조건의 구간 막대기가 0, 1단위로 겹쳐 범주화
coplot(lat ~ long | depth, dat = quakes,
       overlap = 0.1)

# 2단계 : 조건 구간을 5개로 지정하고, 1행 5열이 패널로 조건 그래프 작성
coplot(lat ~ long | depth, data = quakes,
       number = 5, row = 1)


# 패널과 조건막대에 색을 적용하여 조건 그래프 그리기
...
# 3. 기하학적 기법 시각화
# 일반 그래픽 관련 패기지 : 이미지 표현에 중점
# ggplot2 : 기하학적 객체(점, 선, 막대 등)에 미적 특성(색상, 모양, 크기)을 연결.
# 데이터 객체와 그래픽 개체를 서로 분리하고 재사용 가능

# 3.1 qplot함수
# 기하학적 객체에 미적 특성을 매핑하여 그래프에 그려주는 ggplot2패키지에서 제공 함수

# ggplot2 패키지 설치와 실습 데이터 가져오기
...
# 형식 : qplot(x축 ~ y축, data, facet, geom., stat, position,
#              xlim, ylim, log, main, xlab, ylab, asp)

# qplot()함수의 fill과 binwith 속성 적용
...
# facets송성을 사용하여 drv변수값으로 행/열 단위로 패널 생성
# 1단계 : 열 단위 패널 생성
...
# 1단계 : 두 변수로 displ과 hwy변수 사용
qplot(displ, hwy, data = mpg)

# 2단계 : 두 변수로 displ과 hwy변수 사용하여 dry변수에 색상 적용
...
# 4단계 : 모양적용
qplot(wt, mpg, data = mtcars,
      size = qsec, shape = factor(cyl), color = factor(carb))
...
# 8단계 : 산점도 그래프의 평활에 cyl변수의 요인으로 색상 적용하기
...
# 3.2 ggplot()함수
# ggplot()함수는 데이터 속성에 미적요소를 매핑한 후 스케일링 과정을 거쳐서
# 생성된 객체를 + 연산자를 사용하여 미적 요소 맵핑을 새로운 레이어에 상속받아
# 재사용할 수 있도록 지원하는 ggplot2 패키지의 함수
...
# (2) 기하학적 객체 적용
# geom.()함수는 ggplot()함수에서 정의된 미적요소 맵핑 객체를 상속받아서
# 서로 다른 레이어에서 별도로 재사용 할 수 있다.

# geom_line()과 geom_point()를 적용하여 레이어 추가
...
# 4. 지도 공간 기법 시각화
# ggmap패키지
# 구글 관련 패키지를 이용하여 지도 이미지를 서비스 받기 위해서 관련 함수를 실행하면 오류 메시지
# 2019년부터 구글 지도 서비스를 받기 위해서는 구글 지도 API 인증키를 발급받아야 하기 때문에
# 인증키 없이 사용할 수 있는 get_stamenmap() 함수만 이용하여 지도 이미지를 시작화

# 4.1 Stamen Mapls API 이용
# get_stamenmap()함수
...
# 주요속성 :
# bbox : 지도가 그려질 경계상자
# zoom : 확대비율(수치가 잓을수록 중심지역 기준으로 확대)
# maptype : 지도 유형
# crop : 원시 맵 타일을 지정된 경계 사장로 자른다. FALSE인 경우 결과 맵이 지정된 경계상자를 덮는다.
# messaging : 메시지 켜기 끄기
# urlonly : url 반환
# color : 컬러 또는 흑백
