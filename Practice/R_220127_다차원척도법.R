...
# 즉, 항목 사이 거리를 기준으로 하는 자료를 이용하여 항목들의 상대적인 위치를 찾고 거리가 가까운
# 개체들끼리 Group 화 하여 분류할 수 있다.
...
# 1) 계량적(전통적) 다차원 척도법(Classical MDS) : 숫자 데이터로만 구성. 

# eurodist 데이터를 이용하여 유럽 주요 도시 사이의 거리 대상으로 다차원 척도법을 적용하여 그래프로 표현
...
# 그래프를 통해 Paris, Munich 등이 중심에 있고 Lisbon, Stockholm, Athem 등은 중심에서 거리가 좀 있음을 알 수 있다.
# 이 그래프를 통해서 거리가 가까운 도시끼리 Group화가 가능하다

# 2) 비계량적 다차원 척도법 (nonmetric MDS) : 숫자가 아닌 데이터 포함.
